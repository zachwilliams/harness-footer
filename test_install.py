...
class InstallTests(unittest.TestCase):
    ...
    def test_portable_install_preserves_settings_and_forwards_arguments(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            custom = {'target_tokens': 180000, 'claude_show_builtin_status': False}
            ...
            for shell in filter(None, shells):
                for app in ('codex', 'claude'):
                    # Inject PATH after shell startup; Zsh's .zshenv can reset it.
                    command = ('export PATH=' + shlex.quote(env['PATH']) + '; . ' +
                               shlex.quote(str(activation)) + '; ' + app + ' --version "two words"')
                    output = subprocess.check_output([shell, '-c', command], env=env, text=True)
                    self.assertEqual(output.splitlines(), ['--version', 'two words'])
