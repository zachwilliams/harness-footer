...
from harness_footer.common import PACKAGE_DIR, PANE_APP_OPTION, SELF_COMMAND
from harness_footer.launch import INSIDE_TMUX_FLAG, TMUX
...
class IsolatedTmux:
    def __init__(self, root):
        ...
        self.environment = isolated_environment(root)
        self.cache = Path(self.environment['XDG_CACHE_HOME'])
        self.bin = root / 'bin'
        self.bin.mkdir()
        (root / 'claude').mkdir()
    ...
    def test_each_session_gets_its_own_footer(self):
        for name, (app, pane, tokens) in self.panes.items():
            with self.subTest(session=name):
                ...
                self.assertIn('harness_footer status', status_format)
